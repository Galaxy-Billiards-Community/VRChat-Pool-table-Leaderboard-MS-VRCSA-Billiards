...
{
#if EIJIS_MANY_BALLS
	public string PHYSICSNAME = "<color=#FFD700>Advanced V0.5L (max32balls)</color>";
#else
    ...
    */
	...
	void _HANCushionModel(ref Vector3 vel, ref Vector3 angvel, int id, Vector3 N, bool isPocketBounce = false)
	{
        ...
        */
		...
		{
			Debug.Log($"P> {P} | R>{R}");
			Debug.Log($"H> {h} | k_RAIL_HEIGHT_LOWER>{k_RAIL_HEIGHT_LOWER} | ballCenter > {ballCenter}");
			...
			/// For THETA angle
			Debug.Log("<color=white><size=16><b><i>θ</i></b></size></color>: " + θ.ToString("<color=cyan><size=16><i>00.00000000</i></size></color>") + " Rad <size=16>/</size>" + (θ * Mathf.Rad2Deg).ToString("<color=cyan><size=16><i>00.0</i></size></color>") + "Deg");
